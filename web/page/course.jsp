...
<body class="hold-transition skin-purple sidebar-mini">
    <div class="wrapper">
        ...
        <header class="main-header">
            ...
            <a href="all-admin-index.html" class="logo">
                ...
                <span class="logo-lg"><b>课程</b>界面</span>
            </a>
                        ...
                        <div class="table-box">
                            ...
                            <div class="pull-left">
                                <div class="form-group form-inline">
                                    <div class="btn-group">
                                        <%--<a href="<%=basePath%>/page/basicCarInsert.jsp"><button type="button" class="btn btn-default" title="新建"><i class="fa fa-file-o"></i> 添加</button></a>--%>
                                        <a href="<%=basePath%>/CourseController/showCourseAll"><button type="button" class="btn btn-default" title="刷新"><i class="fa fa-refresh"></i> 刷新</button></a>
                                    </div>
                                </div>
                            </div>
                            <!--数据列表-->
                            <table id="dataList" class="table table-bordered table-striped table-hover dataTable">
                                <thead>
                                    <tr>
                                        <th class="text-center sorting">课程号</th>
                                        <th class="text-center sorting">教师编号</th>
                                        <th class="text-center sorting">课程名称</th>
                                        <th class="text-center sorting">性质</th>
                                        <th class="text-center sorting">开课学期</th>
                                        <th class="text-center sorting">开课时间</th>
                                        <th class="text-center sorting">结课时间</th>
                                        <th class="text-center sorting">可选人数</th>
                                        <th class="text-center sorting">类别</th>
                                        <th class="text-center sorting">描述</th>
                                        <th class="text-center sorting">先修知识</th>
                                ...
                                <tbody>
                                    <c:forEach items="${courses}" var="courses">
                                        <tr>
                                            ...
                                            <td>
                                                <fmt:formatDate value="${courses.starttime}" pattern="yyyy/MM/dd HH:mm:ss"/>
                                            </td>
                                            <td>
                                                <fmt:formatDate value="${courses.endtime}" pattern="yyyy/MM/dd HH:mm:ss"/>
                                            </td>
                                            <td>${courses.stunum}</td>
                                            <td>${courses.subtype}</td>
                                            <td>${courses.courdesc}</td>
                                            <td>${courses.earlystudy}</td>
                                            <td>${courses.statue}</td>
                                            <td class="text-center">
                                                <c:if test="${courses.statue!=0}">
                                                     <a href="<%=basePath%>/UnitController/showUnitAll?id=${courses.cid}"><button type="button" class="btn bg-olive btn-xs">查看单元</button></a>
                                                     <a href="<%=basePath%>/PapertestController/showPaper?cid=${courses.cid}"><button type="button" class="btn bg-olive btn-xs">设置试卷</button></a>
                                                    <a href="<%=basePath%>/PapertestController/printPaper?cid=${courses.cid}"><button type="button" class="btn bg-olive btn-xs">打印试卷</button></a>
                                                </c:if>
    ...
    <script>
        ...
        <%--function show(id) {--%>
            <%--$.ajax({--%>
                <%--url: '<%=basePath%>/basicCarController/showdetailCar?id='+id,--%>
                <%--success: function () {--%>
                    <%--alert("成功");--%>
        ...
        function trash(id) {
            $.ajax({
                url: '<%=basePath%>/basicCarController/deleteBasicCar?id='+id,
                success: function () {
                    alert("删除成功");
